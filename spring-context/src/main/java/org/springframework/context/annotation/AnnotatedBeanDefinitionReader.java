...
public class AnnotatedBeanDefinitionReader {
	...
	public AnnotatedBeanDefinitionReader(BeanDefinitionRegistry registry) {
		// AnnotationConfigApplicationContext是BeanDefinitionRegistry的接口实现类
	...
	private <T> void doRegisterBean(Class<T> beanClass, @Nullable String name,
		...
		/*
		 * (1)、根据类生成一个beanDefinition, 具体类型是AnnotatedGenericBeanDefinition
		 * 在当前场景中，beanClass就是传入的Config.class
		 *
		 * (2)、这个AnnotatedGenericBeanDefinition可以理解为一个数据结构，
		 * 该结构中包含了类的一些描述信息。比如scope，lazy等等。这里设置类内部属性metadata、beanClass
		 */
		AnnotatedGenericBeanDefinition abd = new AnnotatedGenericBeanDefinition(beanClass);

		/*
		* 解析abd的ScopeMetadata。在reader初始化时，scopeMetadataResolver就默认初始化为AnnotationScopeMetadataResolver类型了
		...
		*/
		...
		// 在当前场景中，Config类的beanName就为：appConfig
